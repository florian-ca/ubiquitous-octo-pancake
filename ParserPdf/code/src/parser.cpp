...
void parser(string source,string destination,string optionString)
{
	...
	ifstream fichierout(source);
	    ...
	    if(fichierout && fichierin)
	    {
	    	...
	    	bool conclusionNotPassed=false;
	    	...
	    	while(getline(fichierout,s))
	    	{
	    		...
	    		else 
	    		{
			    	...
			    	if (introend!=string::npos)
			    	{
				    	...
				    	else fichierin<<"body:"<<endl;
					}
	
					if (introPassed && s.compare("Introduction")!=0) // écrit dans le body
					{
						fichierin<<s<<endl;
					}


			    	size_t concEnd = s.find("Conclusion");
			    	if (concEnd!=string::npos)
			    	{
			    		
			    		if (option) fichierin<<endl<<"Conclusion:"<<endl<<s.substr(0,concEnd);
			    		else fichierin<<endl<<"</Body>\n<Conclusion>"<<endl<<s.substr(0,concEnd);
			    	...
			    	if (refend!=string::npos)
			    	{
			    		...
			    		else fichierin<<endl<<"</Conclusion>\n<Bibliography>"<<endl<<s.substr(0,refend);
